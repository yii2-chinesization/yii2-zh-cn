...
和 [活动记录（Active
Record）](http://www.yiiframework.com/doc-2.0/guide-db-active-record.html)多种工具。相较于 1.1，Yii 2.0 改进了 AR 的性能，并且通过 ActiveRecord（AR） 和 Query Builder(QB，Q币？) 统一了查询数据的语法。下面的例子展现了你能如何用 AR 或 QB 方便地查询顾客（Customer）的数据。如你所见，两种方式均使用了跟 SQL 语法一脉相承的链式方法调用。
...
和 1.1 一样，Yii 2.0 支持各式各样的缓存选项，从服务器端的缓存，比如[片段缓存](http://www.yiiframework.com/doc-2.0/guide-caching-fragment.html) 和
