...
例如，“Customer” AR类关联
`tbl_customer`表，意味着该类的 `name` 属性自动映射到`tbl_customer`表的`name` 列。
有了活动记录的帮忙，假设变量$customer代表"Customer"类对象，可使用"$customer->name"获取关联数据行的“name”列。上例中，AR提供了一种面向对象的方式来获取数据库存储的数据。但AR的功能不仅于此。
...
默认情况下，ActiveRecord假定存在一个名为`db`的应用组件，提供数据库连接信息和配置信息（默认在应用配置文件内配置）：
[[yii\db\Connection]] 组件通常在应用配置文件中配置：
...
AR的属性映射对应数据行的列数据
对象。特性表现和常规的对象公开属性一样，特性名和对应的列名一样且区分大小写。
...
请注意 updateAll() ，updateAllCounters() 和 deleteAll() 都是静态方法，用整个整个数据表。其他方法只对 AR 对象关联的数据行有效。
...
```php
// 查找所有订单并以客户 ID 和订单 ID 排序，并即时加载 "customer" 表
...
以上，方法 innerJoinWith() 是 joinWith() 的快速方法。
 `INNER JOIN` 类型
...
注意：如果通过 [[yii\db\ActiveQuery::with()]] 使用贪婪加载或使用惰性加载的话，则 on 条件会被放置
SQL 语句的 WHERE 部分没有关联查询。
